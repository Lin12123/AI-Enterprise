...
### 6.5.8 本节开放问题（部分已拍板）
- **Q7 已定**：云平台**先本地部署（Windows 本机，尚未开发）**，未来可能私有云 / 公网 SaaS。→ 客户端对接层用可配置 base_url，标准库 `urllib.request`，屏蔽部署形态差异。
- **Q8 已定**：知识库初始内容**由客户提供**（详见《云平台开发方案》知识库管理模块的录入/导入功能）。
- **Q9 待定**：知识粒度——**结构化标注参数条目**（推荐，可直接喂 B）还是整份标准文档。
- **Q10 已定**：未命中**硬阻断**——不使用默认兜底参数，返回 `need_fetch` 要求先补齐标准再出图。

> 云平台本身的完整开发方案（前后端分离 / 基础数据 / 知识库管理 / 插件产物管理）见独立文档 [`docs/cloud_platform_design.md`](docs/cloud_platform_design.md)。
